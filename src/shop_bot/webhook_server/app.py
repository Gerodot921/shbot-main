...
ALL_SETTINGS_KEYS = [
    ...
    "telegram_bot_username", "admin_telegram_id", "lava_api_key",
    "lava_offer_id", "lava_webhook_api_key", "receipt_email", "cryptobot_token",
...
def create_webhook_app(bot_controller_instance):
    ...
    def settings_page():
        if request.method == 'POST':
            ...
            for key in ALL_SETTINGS_KEYS:
                if key in ['panel_password', 'force_subscription', 'trial_enabled', 'enable_referrals']:
                    continue
    ...
    def lava_webhook_handler():
        try:
            ...
            expected_webhook_key = get_setting("lava_webhook_api_key") or get_setting("lava_api_key")

            if expected_webhook_key and not compare_digest(webhook_key, expected_webhook_key):
                logger.warning("Lava.top webhook: invalid X-Api-Key header.")
            ...
            if event_type in ("payment.success", "subscription.recurring.payment.success"):
                ...
                if metadata and bot is not None and payment_processor is not None:
                    ...
                    if loop and loop.is_running():
                        ...
                        logger.error("Lava.top webhook: Event loop is not available!")
            return 'OK', 200
