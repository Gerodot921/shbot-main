...
def _complete_pending_transaction(payment_id: str, amount_currency: float, currency_name: str, payment_method: str) -> dict | None:
    try:
        with sqlite3.connect(DB_FILE) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            ...
            if not transaction:
                logger.warning(f"Payment webhook: Received event for unknown or completed payment_id: {payment_id}")
                return None
            
            
            cursor.execute(
                "UPDATE transactions SET status = 'paid', amount_currency = ?, currency_name = ?, payment_method = ? WHERE payment_id = ?",
                (amount_currency, currency_name, payment_method, payment_id)
            )
            conn.commit()
...
def find_and_complete_ton_transaction(payment_id: str, amount_ton: float) -> dict | None:
    return _complete_pending_transaction(payment_id, amount_ton, 'TON', 'TON')

def find_and_complete_lava_transaction(payment_id: str, amount_rub: float, currency_name: str = 'RUB') -> dict | None:
    return _complete_pending_transaction(payment_id, amount_rub, currency_name, 'Lava.top')
